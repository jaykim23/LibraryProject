	...
	function writeCheck() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				location.href = "./freeboard/freeboard_list?page="+page+"&category="+category+"&search="+search";
			},
			error : function() {
				alert("에러");
			}
...
<body>
	...
	<section>
		...
		<form action="freeboard_modify" id="modifyForm" name="modifyForm" method="post"
			enctype="multipart/form-data">
			<table>
				<input type="hidden" name="page" value="${map.page }">
				<input type="hidden" name="bid" value="${map.freeboardDto.bid }">
				<input type="hidden" name="category" value="${map.category }">
				<input type="hidden" name="search" value="${map.search }">
				<input type="hidden" name="file" value="${map.freeboardDto.fileName}">
				<colgroup>
					<col width="15%">
					<col width="85%">
				</colgroup>
				<tr>
					...
					<td><input type="text" name="bname" id="bname"
						value="${map.freeboardDto.bname}"  readonly></td>
				</tr>
				<tr>
					<th>제목</th>
					<td><input type="text" name="btitle" id="btitle"
						value="${map.freeboardDto.btitle}"></td>
				</tr>
				<tr>
					<th>내용</th>
					<td><textarea name="bcontent" cols="50" rows="10"
							id="bcontent">${fn:replace(map.freeboardDto.bcontent,br,cn)}</textarea></td>
				</tr>
				<tr>
					<!-- https://kuzuro.blogspot.com/2018/10/12.html 참고사이트 -->
					<th>이미지 표시</th>
					<c:choose>
						<c:when test="${map.freeboardDto.fileName == null || map.freeboardDto.fileName == ''}">
							<td>
						...
						<c:otherwise>
							<td>
								<input type="file" name="file" id="file" value="${map.freeboardDto.fileName}">
								<img src="upload/${map.freeboardDto.fileName }" width="100px" height="100px" />
								<a href="upload/${map.freeboardDto.fileName }" download>이미지 다운로드</a>
							</td>
						</c:otherwise>
					</c:choose>
				</tr>
			...
			<!-- onclick="modifyCheck()" -->
				<button type="button" onclick="modifyCheck()"  class="write">수정완료</button>
				<button type="button" class="cancel"
					onclick="javascript:location.href='./freeboard_list'">취소</button>
			</div>
		</form>
