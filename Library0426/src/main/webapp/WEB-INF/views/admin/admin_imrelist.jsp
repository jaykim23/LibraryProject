		...
		//컨트롤러에 보내서 저장하기 위해서 ajax를 쓰는것이다
			...
			data:{
	 		...
	 		success: function(data){
				//삭제여부
				alert("반납 완료했습니다.");
				location.href="./admin_Imminentreservationlist";
			},
			error:function(){
				alert("삭제 실패!"); 
			}
...
<body>
  ...
  <section>
    ...
    <div class="wrapper">
      <form action="./admin_Imminentreservationlist" name="search" method="post">
    ...
    <ul class="page-num">
			...
			<c:if test="${map.page!=1}">
				<a href="./admin_Imminentreservationlist?category=${map.category }&search=${map.search }&page=1"><li class="first"></li></a>
			</c:if>
			<c:choose>
				...
				<c:otherwise>
					<a href="./admin_Imminentreservationlist?category=${map.category }&search=${map.search }&page=${map.page-1 }"><li class="prev"></li></a>
				</c:otherwise>
			</c:choose>
			...
			<c:forEach var="nowpage" begin="${map.startpage }"
				...
				<c:choose>
					...
					<c:otherwise>
						<a
							href="./admin_Imminentreservationlist?category=${map.category }&search=${map.search }&page=${nowpage }">
							<li class="num"><div>${nowpage }</div></li>
						</a>
					</c:otherwise>
				</c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
					<a
						href="./admin_Imminentreservationlist?category=${map.category }&search=${map.search }&page=${map.page+1 }"><li class="next"></li></a>
				</c:otherwise>
			</c:choose>
			...
			<c:if test="${map.page!=map.maxpage}">
				<a
					href="./admin_Imminentreservationlist?category=${map.category }&search=${map.search }&page=${map.maxpage}"><li class="last"></li></a>
			</c:if>
		</ul>
